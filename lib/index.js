...
import {
  ...
  renderMemory, memorySummary, pruneMemory, applyReview, exportMarkdown,
} from "./memory.js";
...
function apply(ctx, config) {
  ...
  async function saveDoc(exec, target, doc) {
    const pruned = pruneMemory(doc, { changelogCap: config.changelogCap, lessonTtlDays: config.lessonTtlDays });
    ...
    ctx.effect(() =>
      ctx.systemPrompt.section({
        ...
        text: "A personal memory lives in the session workspace at .dsh-memory-setup/memory.json. " +
          ...
          "and follow them. Record new conventions with memory_project. When something fails, formalize the fix with memory_review " +
          "so the same mistake is not repeated (duplicates are merged automatically).",
      }),
    );
  }
