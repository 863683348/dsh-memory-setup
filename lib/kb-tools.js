...
import { kbAdd, kbSearch, kbList, kbRemove, kbHit, kbSummary, renderKB } from "./knowledge.js";
...
export function registerKnowledgeTools(ctx, storage) {
  ...
  ctx.tools.register(defineTool({
    ...
    execute: async (args, exec) => {
      const { kb, target } = await storage.loadKB(exec);
      const results = kbSearch(kb, args.query, { limit: Math.max(1, Math.min(20, args.limit || 5)) });
      if (results.length) {
