...
export function isValidMemory(doc) {
  return !!doc && (doc.version === MEMORY_VERSION || doc.schemaVersion === 2) && typeof doc.preferences === "object" && Array.isArray(doc.projectConventions) && Array.isArray(doc.workflows) && Array.isArray(doc.lessons);
}
...
export function renderMemory(doc, maxChars = 6000) {
  ...
  if (d.lessons.length) {
    ...
    for (const l of d.lessons) out.push("- ERROR: " + l.error + " -> FIX: " + l.fix + (l.evidence ? " (evidence: " + l.evidence + ")" : "") + (l.health === "failing" ? " ⚠️FIX-NOT-WORKING" : l.health === "resolved" ? " ✅resolved" : ""));
  }
  let text = out.join("\n");
...
// ---- v0.8: schema migration, merge, import/export ----

/** Schema migration: normalize any old doc into the current shape (v2). Pure. */
export function migrateMemory(doc) {
  if (!doc || typeof doc !== "object") return emptyMemory();
  const d = structuredClone(doc);
  if (!d.schemaVersion || d.schemaVersion < 2) {
    d.schemaVersion = 2;
    d.preferences = (d.preferences && typeof d.preferences === "object") ? d.preferences : {};
    d.projectConventions = Array.isArray(d.projectConventions) ? d.projectConventions : [];
...
/** Merge two memory docs. conflict: 'newer' or 'both'. Pure. */
export function mergeMemories(a, b, { conflict = "newer" } = {}) {
  const A = migrateMemory(a);
  const B = migrateMemory(b);
  const out = emptyMemory();
  out.schemaVersion = 2;
  const pick = (key) => conflict === "newer" ? ((B.updatedAt || "") > (A.updatedAt || "") ? B.preferences[key] : A.preferences[key]) : (A.preferences[key] || B.preferences[key]);
  for (const k of ["language", "codeStyle", "tools", "notes"]) {
    const v = pick(k);
    if (String(v).trim()) out.preferences[k] = v;
  }
  const seen = new Set();
  for (const list of [A.projectConventions, B.projectConventions]) {
    for (const c of list || []) {
      const key = (c.detail || c.id || "") + (c.type || "");
      if (!seen.has(key)) { seen.add(key); out.projectConventions.push(structuredClone(c)); }
    }
  }
  const seenW = new Set();
  for (const list of [A.workflows, B.workflows]) {
    for (const w of list || []) {
      if (!seenW.has(w.id)) { seenW.add(w.id); out.workflows.push(structuredClone(w)); }
    }
  }
  const seenL = new Set();
  for (const list of [A.lessons, B.lessons]) {
    for (const l of list || []) {
      if (!seenL.has(l.id)) { seenL.add(l.id); out.lessons.push(structuredClone(l)); }
    }
  }
  out.updatedAt = new Date().toISOString();
  out.changelog.push({ ts: out.updatedAt, action: "merge", path: "-", summary: "merged two memories" });
  return out;
}

/** Validate an imported JSON memory payload. Pure. */
export function memoryFromJSON(text) {
  let parsed;
  try { parsed = JSON.parse(text); } catch { throw new Error("invalid JSON"); }
  if (!parsed || typeof parsed !== "object") throw new Error("not a valid memory document");
  const declared = parsed.version === MEMORY_VERSION || parsed.schemaVersion === 2;
  if (!declared) throw new Error("not a valid memory document");
  const migrated = migrateMemory(parsed);
  if (!isValidMemory(migrated)) throw new Error("not a valid memory document");
  return migrated;
}
// ---- v0.9: lesson health + relevance selection ----

/**
 * Evaluate lesson health. Pure:
 *  - 'failing': hits>=3 and lastSeenAt within ineffectiveWindowDays (fix not working)
 *  - 'resolved': not seen in resolvedAfterDays
 *  - 'active': otherwise
 */
export function updateLessonHealth(doc, { now = Date.now(), ineffectiveWindowDays = 7, resolvedAfterDays = 30 } = {}) {
  const d = doc && isValidMemory(doc) ? structuredClone(doc) : emptyMemory();
  const nowIso = new Date(now).toISOString();
  for (const l of d.lessons) {
    const hits = l.hits ?? 1;
    const last = l.lastSeenAt || l.createdAt || null;
    if (hits >= 3 && last) {
      const days = (now - new Date(last).getTime()) / 86400000;
      l.health = days <= ineffectiveWindowDays ? "failing" : (days > resolvedAfterDays ? "resolved" : "active");
    } else if (hits >= 3) {
      l.health = "failing";
    } else if (last && (now - new Date(last).getTime()) / 86400000 > resolvedAfterDays) {
      l.health = "resolved";
    } else {
      l.health = l.health || "active";
    }
    void nowIso;
  }
  return d;
...
/** Token-based relevance of a text against a query (BM25-lite). Pure. */
function relevance(query, text) {
  const qt = new Set(String(query).toLowerCase().split(/[^\p{L}\p{N}]+/u).filter(Boolean));
  const tt = String(text).toLowerCase().split(/[^\p{L}\p{N}]+/u).filter(Boolean);
  let score = 0;
  for (const t of qt) { let n = 0; for (const x of tt) if (x === t) n++; if (n) score += n; }
  return score;
}

/** Select the most relevant conventions/lessons for a focus query. Pure. */
export function selectRelevant(doc, query, { limit = 5 } = {}) {
  const d = doc && isValidMemory(doc) ? doc : emptyMemory();
  if (!String(query || "").trim()) return { conventions: [], lessons: [], score: 0 };
  const conv = (d.projectConventions || []).map((c) => ({ item: c, score: relevance(query, c.detail) }));
  const lessons = (d.lessons || []).map((l) => ({ item: l, score: relevance(query, l.error + " " + l.fix) }));
  return {
    conventions: conv.filter((x) => x.score > 0).sort((a, b) => b.score - a.score).slice(0, limit).map((x) => x.item),
    lessons: lessons.filter((x) => x.score > 0).sort((a, b) => b.score - a.score).slice(0, limit).map((x) => x.item),
    score: 1,
  };
}
// ---- v1.0: tiers, redaction, audit ----

/** Set the tier (hot/warm/cold) of an entry. Pure. */
export function setTier(doc, kind, id, tier) {
  const d = doc && isValidMemory(doc) ? structuredClone(doc) : emptyMemory();
  if (!["hot", "warm", "cold"].includes(tier)) throw new Error("tier must be hot|warm|cold");
  const list = kind === "lesson" ? d.lessons : kind === "workflow" ? d.workflows : kind === "convention" ? d.projectConventions : null;
  if (!list) throw new Error("kind must be lesson|workflow|convention");
  const e = list.find((x) => x.id === id);
  if (!e) throw new Error("entry not found: " + id);
  e.tier = tier;
  d.updatedAt = new Date().toISOString();
  d.changelog.push({ ts: d.updatedAt, action: "tier", path: kind + ":" + id, summary: tier });
  return d;
}

/** Copy a doc with sensitive preference values redacted. Pure. */
export function redactDoc(doc, sensitiveKeys = []) {
  const d = doc && isValidMemory(doc) ? structuredClone(doc) : emptyMemory();
  const keys = new Set(sensitiveKeys);
  for (const k of Object.keys(d.preferences || {})) {
    if (keys.has(k)) d.preferences[k] = "[redacted]";
  }
  return d;
}

/** Build a human-readable audit report from a doc + expected integrity. Pure. */
export function auditReport(doc, expectedHash, actualHash) {
  const d = doc && isValidMemory(doc) ? doc : emptyMemory();
  const out = [];
  out.push("Memory audit");
  out.push("- integrity: " + (expectedHash && actualHash === expectedHash ? "OK (" + actualHash.slice(0, 12) + ")" : actualHash ? "MISMATCH (expected " + String(expectedHash).slice(0, 12) + ", got " + actualHash.slice(0, 12) + ")" : "not tracked"));
  out.push("- schemaVersion: " + (d.schemaVersion || 1));
  out.push("- entries: " + (Object.values(d.preferences || {}).filter((x) => String(x).trim()).length + (d.projectConventions || []).length + (d.workflows || []).length + (d.lessons || []).length));
  out.push("- changelog entries: " + (d.changelog || []).length);
  for (const c of (d.changelog || []).slice(-20)) out.push("  - " + (c.ts || "?") + " [" + c.action + "] " + c.path + ": " + c.summary);
  return out.join("\n");
}

// ---- v1.1: CLAUDE.md import ----

/** Parse CLAUDE.md-style text into convention details. Pure. */
export function parseClaudeMd(text) {
  ...
  for (const raw of String(text).split(/\r?\n/)) {
    ...
    if (item && item.length >= 8) out.push({ type: "imported", pattern: "claude-md", detail: item, evidence: "CLAUDE.md" });
  }
  return out;
}

// ---- v1.2: annotations + bundle ----

/** Annotate an entry with owner/purpose. Pure. */
export function annotateEntry(doc, kind, id, fields) {
  const d = doc && isValidMemory(doc) ? structuredClone(doc) : emptyMemory();
  const list = kind === "lesson" ? d.lessons : kind === "workflow" ? d.workflows : kind === "convention" ? d.projectConventions : null;
  if (!list) throw new Error("kind must be lesson|workflow|convention");
  const e = list.find((x) => x.id === id);
  if (!e) throw new Error("entry not found: " + id);
  if (fields.owner) e.owner = fields.owner;
  if (fields.purpose) e.purpose = fields.purpose;
  d.updatedAt = new Date().toISOString();
  d.changelog.push({ ts: d.updatedAt, action: "annotate", path: kind + ":" + id, summary: fields.owner || fields.purpose || "" });
  return d;
}

/** Bundle memory + knowledge + snapshots index into one object. Pure. */
export function buildBundle(doc, kb, snapshots, extra = {}) {
  return { bundleVersion: 1, exportedAt: new Date().toISOString(), memory: migrateMemory(doc), knowledge: kb, snapshots: snapshots || [], ...extra };
}

/** Validate a bundle payload. Pure. */
export function parseBundle(text) {
  let p;
  try { p = JSON.parse(text); } catch { throw new Error("invalid JSON"); }
  if (!p || p.bundleVersion !== 1 || !isValidMemory(migrateMemory(p.memory))) throw new Error("not a valid bundle");
  return p;
}
